...
interface Projectile {
  active: boolean; x: number; y: number; color: string;
}
...
export class CoreballEngine {
  ...
  private dir: 1 | -1 = 1;
  private pins: Pin[] = [];
  private projectile: Projectile = { active: false, x: 0, y: 0, color: "#0e8af0" };
  private particles: Particle[] = [];
  private shake = 0;
  private flash = 0;
  private attachPulseTick = 0;
  ...
  resize(w: number, h: number): void {
    ...
    this.headR = Math.max(6, this.R * scale);
    this.shaftLen = this.R * SHAFT_RATIO;
    this.rh = this.R + this.shaftLen;
    this.projSpeed = Math.max(w, h) * 2.2;
  }
  ...
  fire(): void {
    ...
    this.projectile.y = this.h + this.headR + 8;
  ...
  private update(dt: number): void {
    if (this.state === "playing" || this.state === "ready") {
      ...
      this.attachPulseTick = (this.attachPulseTick + 1) % 2;
    } else {
      this.speed = 0;
    }

    for (const pin of this.pins) pin.pulse = Math.max(0, pin.pulse - dt * 2.2);
  ...
  private stepProjectile(dt: number): void {
    ...
    const dx = this.cx - this.projectile.x;
    const dy = this.cy - this.projectile.y;
    ...
    const nx = dx / dist;
    const ny = dy / dist;
    this.projectile.x += nx * step;
    this.projectile.y += ny * step;
    this.checkCollision();
  }
  ...
  private attachProjectile(): void {
    ...
    this.pins.push({ angle: attachAngle, color: p.color, pulse: 1 });
    this.attached++;
    p.active = false;
  ...
  private checkCollision(): void {
    ...
    for (const pin of this.pins) {
      ...
      if (dh < threshold) return this.fail(hx, hy);
      if (distToSegment(p.x, p.y, ax, ay, hx, hy) < shaftThreshold) return this.fail(hx, hy);
      const awayX = p.x - this.cx;
      const awayY = p.y - this.cy;
      const awayLen = Math.hypot(awayX, awayY) || 1;
      const trailX = p.x + (awayX / awayLen) * this.shaftLen;
      const trailY = p.y + (awayY / awayLen) * this.shaftLen;
      if (distToSegment(hx, hy, p.x, p.y, trailX, trailY) < shaftThreshold) return this.fail(hx, hy);
    }
  }

  private fail(hx: number, hy: number): void {
    if (this.state !== "playing") return;
    this.projectile.active = false;
    this.shake = 1;
    this.flash = 0.85;
  ...
  private draw(): void {
    ...
    for (const pin of this.pins) {
      ...
      const headX = cdx * this.rh;
      const headY = cdy * this.rh;
    ...
    if (this.projectile.active) {
      ...
      const nx = dx / d;
      const ny = dy / d;
      const tx = this.projectile.x + nx * (this.headR * 0.9 + this.shaftLen);
      const ty = this.projectile.y + ny * (this.headR * 0.9 + this.shaftLen);
      ctx.beginPath();
      ctx.moveTo(this.projectile.x + nx * this.headR * 0.9, this.projectile.y + ny * this.headR * 0.9);
      ctx.lineTo(tx, ty);
      ctx.lineWidth = Math.max(2, this.headR * 0.26);
      ctx.lineCap = "round";
      ctx.strokeStyle = "#98a3ba";
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(this.projectile.x, this.projectile.y, this.headR, 0, Math.PI * 2);
